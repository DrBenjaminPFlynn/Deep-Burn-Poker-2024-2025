...
date: "Sex is good, they say, but poker lasts longer. ~ Al Alvarez"
output: 
...
for (PLAYER in PLAYERS) {
  ...
  df.inter[["Runing.Total"]] <- df.inter[1, "Total.Points"]
  
  ## Add label to most recent game for ggplot
  df.inter[["LABEL"]] <- ""
  df.inter[nrow(df.inter),][["LABEL"]] <- PLAYER
  
  if (nrow(df.inter) > 2) {
    for (GAME.NO in seq(2, nrow(df.inter))) {
      df.inter[GAME.NO, ][["Runing.Total"]] <- df.inter[GAME.NO-1, ][["Runing.Total"]] + df.inter[GAME.NO, ][["Total.Points"]] # More than two games
    }
  }else{
    
    if (nrow(df.inter) == 2) {
      df.inter[2,][["Runing.Total"]] <- df.inter[1, ][["Total.Points"]] + df.inter[2, ][["Total.Points"]] # only two games played
    }
    
  }
  
  df.inter <- select(df.inter, Player, Game, Runing.Total, LABEL)
